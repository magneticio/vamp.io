...
node("mesos-slave-vamp.io") {
  withEnv(['VAMP_VERSION=0.9.4']) {
    ...
    stage('Deploy') {
      if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
        sh script: '''
        cat config/blueprint.yaml | sed s/VERSION/$VAMP_VERSION/ | curl -d @- http://10.20.0.100:8080/api/v1/blueprints?validate_only=true -H 'Content-type: application/x-yaml'
        '''
      }
    }
    stage('Smoke')
